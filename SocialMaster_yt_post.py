from SocialMaster_yt_constants import *
from SocialMaster_yt_logging import Log
from selenium.webdriver.support import expected_conditions as EC
from SocialMaster_yt_exceptions import *
...
class Upload:
    ...
    def upload(
        ...
        try:
            dismiss = self.driver.find_element(By.XPATH, "//ytcp-button[contains(@id, 'dismiss-button')]")
            dismiss.click()
        except selenium_exceptions.NoSuchElementException:
            pass

        input_file = WebDriverWait(self.driver, 120).until(ec.presence_of_element_located((By.XPATH, INPUT_FILE_VIDEO)))
        input_file.send_keys(get_path(file))
        sleep(self.timeout)
        ...
        title_field = WebDriverWait(modal, 60).until(
        ...
        for _ in range(len(title_field.text) + 20):
        ...
        public_main_button = WebDriverWait(modal, 60).until(
            EC.visibility_of_element_located((By.NAME, PUBLIC_BUTTON))
        )
